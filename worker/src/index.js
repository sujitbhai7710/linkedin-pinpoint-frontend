  ...
  async scheduled(event, env, ctx) {
    ...
    try {
      ...
      if (scrapeNeeded) {
        ...
        console.log(`Scrape succeeded! Puzzle #${data.number} (${data.answer})`);

        // NOTE: Article generation has been MOVED to GitHub Actions (fetch-data.js).
        // The worker now only stores raw puzzle data (clues, answer, solutions).
        // GitHub Actions will call NVIDIA during the build to generate the article,
        // with a 5-model fallback chain. This avoids Worker wall-clock limits and
        // gives the build 6 hours to try multiple models.
        console.log('Storing raw puzzle data in DB (article will be generated by GitHub Actions)...');
        await env.DB.prepare(
          ...
          data.answer,
          null,  // explanation — will be generated by GitHub Actions during build
        ...
        dataToCommit = {
          ...
          clues: data.clues,
          answer: data.answer,
          explanation: null,  // no article yet — GitHub Actions will generate it
    ...
    try {
      ...
      if (path === '/scrape') {
        ...
        try {
          ...
          // NOTE: Article generation has been MOVED to GitHub Actions.
          // Worker only stores raw puzzle data. Explanation is set to null
          // and will be generated during the GitHub Actions build.
          console.log('Storing raw puzzle data (article will be generated by GitHub Actions)...');
          ...
          await env.DB.prepare(
            ...
            JSON.stringify(data.clues),
            data.answer,
            null,  // explanation — GitHub Actions will generate it
            JSON.stringify(data.solutions)
          ).run();

          return new Response(JSON.stringify({
            success: true,
            message: 'Scraped from LinkedIn Official Voyager API (article will be generated by GitHub Actions)',
            data: data
          }), { headers: corsHeaders });
        } catch (err) {
          return new Response(JSON.stringify({
      ...
      if (cronTestMatch) {
        ...
        try {
          ...
          if (dbResult) {
            ...
            // NOTE: Article generation has been MOVED to GitHub Actions.
            // Worker stores raw puzzle data only; GitHub Actions generates the article.
            step('explanation', 'skipped', { reason: 'Article generation moved to GitHub Actions build step' });
            data.explanation = null;
            ...
            dataToCommit = {
              ...
              clues: data.clues,
              answer: data.answer,
              explanation: null,  // GitHub Actions will generate it
              solutions: data.solutions
            };
          }
