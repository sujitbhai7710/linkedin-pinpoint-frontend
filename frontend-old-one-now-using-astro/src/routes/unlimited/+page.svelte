<script lang="ts">
        ...
        // Puzzle data map from server (loaded from static JSON at build time)
        const puzzleDataMap = data.puzzleDataMap;
        ...
        function checkAnswer(guess: string, answer: string): boolean {
                ...
                if (guessWords.length >= 2 && answerWords.length >= 2) {
                        ...
                        if (matchCount === guessWords.length && Math.abs(answerWords.length - guessWords.length) <= 1) {
                                return true;
                        }
                }
        ...
        function shuffleDeck() {
                // Filter to only puzzles that have a corresponding answer in puzzleDataMap
                const playablePuzzles = data.allPuzzles.filter(p => puzzleDataMap[p.number]);
                if (playablePuzzles.length === 0) {
                        deck = [];
                        return;
                }
        ...
        function saveLocalStats() {
                try {
                        localStorage.setItem('pinpoint-unlimited-local-stats', JSON.stringify(localStats));
                } catch (e) {
                        // Ignore
                }
        }
        ...
        async function startRound() {
                ...
                // Look up puzzle data from the pre-loaded map (static JSON)
                const puzzleData = puzzleDataMap[puzzleSummary.number];
                if (puzzleData) {
                        answer = puzzleData.answer;
                        clues = puzzleData.clues;
                } else {
                        // No data available for this puzzle, skip to next
                        loading = false;
                        startRound();
                        return;
                }
        ...
        function submitGuess() {
                ...
                if (checkAnswer(guess, currentRound.answer)) {
                        ...
                        if (localStats.currentStreak > localStats.bestStreak) {
...
<div class="game-container">
                ...
                {:else}
                        ...
                        <div class="game-result {currentRound.state}">
                                ...
                                {#if currentRound.state === 'won'}
                                        <p class="text-sm text-secondary">Solved with {currentRound.revealedClues} clue{currentRound.revealedClues !== 1 ? 's' : ''}</p>
                                {:else}
                                        <p class="text-sm text-secondary">Better luck on the next one!</p>
                                {/if}
        ...
        {:else}
                <div class="text-center" style="padding: 3rem 0;">
                        ...
                        <p class="text-secondary">Check back later for more puzzles.</p>
                </div>
        {/if}
</div>
