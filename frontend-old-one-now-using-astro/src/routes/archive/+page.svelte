<script lang="ts">
        ...
        // Handle hash fragment on mount — e.g., /archive#2026-05-17
        // Also fall back to ?date= query param for backwards compatibility
        onMount(() => {
                const hash = window.location.hash.slice(1); // Remove #
                if (hash && /^\d{4}-\d{2}-\d{2}$/.test(hash)) {
                        selectDate(hash);
                } else {
                        // Backwards compat: check for ?date= query param
                        const params = new URLSearchParams(window.location.search);
                        const dateParam = params.get('date');
                        if (dateParam) {
                                // Replace ?date= with hash and clean the URL
