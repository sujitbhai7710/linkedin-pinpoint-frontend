<script lang="ts">
	...
	// Parse puzzle dates from the data
	let puzzleDates = $state<Set<string>>(new Set(Object.keys(data.puzzleDates)));

	// Build a lookup for puzzle info by date
...
<section class="section" aria-labelledby="archive-heading">
	<div class="container">
		<div class="section-header">
			...
			<p class="text-secondary">Browse all past LinkedIn Pinpoint puzzles by date</p>
		</div>
			...
			<div class="article-main">
				...
				{:else if selectedError}
					<div class="mt-3 error-state">
						<h3>{selectedError}</h3>
						<p class="text-secondary">Try selecting a different date from the calendar.</p>
					</div>
				...
				{:else}
					<div class="mt-3 text-center" style="padding: 3rem 1rem;">
						<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="var(--color-text-tertiary)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="margin: 0 auto 1rem;" aria-hidden="true"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
						<h3 class="text-secondary">Select a Date</h3>
						<p class="text-sm text-secondary mt-1">Click on a highlighted date in the calendar to view that day's puzzle answer.</p>
					</div>
				{/if}
			</div>
